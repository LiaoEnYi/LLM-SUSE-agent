...
const sendMessage = () => {
  ...
  // 使用SSE调用doChatWithManus接口
  const eventSource = new EventSource(`/api/suseAgent/manus/chat?message=${encodeURIComponent(message)}`)
...
onMounted(() => {
  ...
  // 欢迎消息
  messages.value.push({ type: 'ai', content: '您好！我是AI超级智能体，有什么可以帮助您的？' })
  scrollToBottom()
})
</script>
